...
public class NotificationUtil {
    ...
    public String getLocalizationMessages(String tenantId, RequestInfo requestInfo, String module) {
        final StringBuilder uri = getUri(tenantId, requestInfo, module);
        log.info("localization url: {}", uri.toString());
    ...
    public String getShortnerURL(String actualURL) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(res)) {
                log.error("URL_SHORTENING_ERROR: Empty response received for url: {}", actualURL);
                return actualURL;
            }
            return res;
        } catch (Exception e) {
            log.error("URL_SHORTENING_ERROR: Failed to shorten url: {}", actualURL, e);
            return actualURL;
        }
    }
